...
def interpolation(left_data, right_data, t, method='linear', return_first_key=True):
    ...
    if method == 'linear':
        ...
        for frame_idx in range(1, t + 1):
            data_between = np.zeros_like(left_data)
            for joint_idx in range(num_joints):
                ...
                # interpolate at frame_idx / t
                interp_time = frame_idx / t
                interp_quaternion = slerp_function([interp_time])
                # convert back to quaternion and store
                data_between[joint_idx] = interp_quaternion.as_quat()[0]
            res.append(data_between)
...
def concatenate_two_motions(motion1, motion2, last_frame_index, start_frame_indx, between_frames, searching_frames=20, method='interpolation'):
    ...
    ########## Code Start ############
    win1_start = max(0, last_frame_index - searching_frames)
    win1_end = min(len(motion1.local_joint_rotations), last_frame_index + searching_frames)
    win2_start = max(0, start_frame_indx - searching_frames)
    win2_end = min(len(motion2.local_joint_rotations), start_frame_indx + searching_frames)

    search_win1 = motion1.local_joint_rotations[win1_start:win1_end] # search source
    search_win2 = motion2.local_joint_rotations[win2_start:win2_end] # search target
    ...
    real_i, real_j = win1_start + i, win2_start + j
    
    # align root positions of motion_2(real_j) to the root positions of motion_1(real_i)
    root_offset = motion1.local_joint_positions[real_i, 0] - motion2.local_joint_positions[real_j, 0]
    for frame in range(real_j, len(motion2.local_joint_positions)):
        motion2.local_joint_positions[frame, 0] += root_offset
    
    pose1_pos = motion1.local_joint_positions[real_i]
    pose1_rot = motion1.local_joint_rotations[real_i]
    pose2_pos = motion2.local_joint_positions[real_j]
    pose2_rot = motion2.local_joint_rotations[real_j]

    between_local_pos = interpolation(pose1_pos, pose2_pos, between_frames, 'linear')
    between_local_rot = interpolation(pose1_rot, pose2_rot, between_frames, 'slerp')
